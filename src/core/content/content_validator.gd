...
## AC-13 (TR-part-013): every non-`&""` `active_skill_id` resolves to a mounted Move
## DB entry, and every non-`&""` `passive_id` to a mounted Passive DB entry. `&""`
## (no reference) is skipped, never flagged. Resolves against the injected id sets
## (ADR-0003 — StringName IDs, DI), never a live autoload.
func _check_referential_integrity(part: PartDef) -> void:
	if part.active_skill_id != &"" and not _move_ids.has(part.active_skill_id):
		_error(&"content_dangling_skill_ref", {"id": part.id, "active_skill_id": part.active_skill_id})
