## PartDB — loads the PartCatalog into a read-only O(1) index and vends defs.
##
## Registered as an autoload (final name/order in ADR-0004); consumers read
## project-wide via `PartDB.get_part(id)`. This is a THIN HOST (ADR-0004): it does
...
## `load_catalog` explicitly — never `_ready`.
##
...
## Index of every loaded part, keyed by its unique id. O(1) `Dictionary.get`
...
## contract on [method get_part].
func has_part(id: StringName) -> bool:
	return _by_id.has(id)

## Returns the shared frozen [PartDef] for [param id], or null for any unknown
## id (including &"" and a null argument, both of which coerce to a missing key).
##
## The `-> PartDef` annotation does NOT protect callers from null: GDScript object
## types are nullable, so this compiles and runs while silently delivering null.
## Callers MUST null-check, or guard with [method has_part].
