## PassiveCatalog — the explicit manifest of every PassiveDef that ships in a build.
##
## Authored as a single .tres asset (assets/data/catalogs/passive_catalog.tres).
...
##
## To add a new passive: create its .tres file under assets/data/passives/, then
...
## All PassiveDef entries shipped in this build. Typed so the inspector enforces
## that only PassiveDef resources are dragged in.
@export var entries: Array[PassiveDef] = []
