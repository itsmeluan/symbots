## PartDB autoload — thin host delegating to the PartDB core class (ADR-0004 §1 slot 3).
##
## Registered as autoload "PartDB"; consumers read via `PartDB.get_part(id)`,
## `PartDB.has_part(id)`. This script is the AUTOLOAD WRAPPER only — all logic lives
## in `src/core/content/part_db.gd`. No logic is moved here.
##
...
##
## load_catalog delegates directly to the core PartDB instance (`_db`).
extends Node
...
func load_catalog(catalog: PartCatalog, log: LogSink) -> bool:
	return _db.load_catalog(catalog, log)

## Delegate: true when [param id] is present in the index.
func has_part(id: StringName) -> bool:
	return _db.has_part(id)

## Delegate: returns the shared frozen [PartDef] for [param id], or null for unknown id.
## Callers MUST null-check or guard with has_part().
func get_part(id: StringName) -> PartDef:
	return _db.get_part(id)
