...
## pollute the production global class registry. Mirrors the Assembly / Part-DB spies.
extends LogSink
...
## True if any warn's detail names [param tier] under the "tier" key (EC-SYN-12/13).
func warned_tier(tier: StringName) -> bool:
	for w in warns:
		if w["detail"].get("tier", &"") == tier:
			return true
	return false
