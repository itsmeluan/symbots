...
| 3 | Parts — silhouette slots | Part-family | Render family per slot × rarity × manufacturer for CHASSIS / HEAD / ARMS / LEGS / WEAPON (the four silhouette-extending slots + weapon). Modular part-render pipeline; each must change the outline when broken (Principle 4). | part-database.md, art-bible §3.3/§8.1 | Needed |
...
| 5 | Manufacturer surface vocabularies | Style-set | 3 manufacturer surface finishes (**Ironclad** / **Scrapjaw** / **Boltwell**) + **wild** (evolved-organic, biome-adaptive, no manufacturer tag). Manufacturer = surface + set-synergy identity **only**, orthogonal to role (§3.2 mass) and element (§4.2 color). **Names canon — no rename pending** (placeholder labels retired 2026-07-18). | art-bible §3.8 | **Reference (resolved)** — governs part art (#3/#4); no standalone asset |
...
- ~~**Blocker to clear before faction art**: the 4 faction names~~ **RESOLVED 2026-07-18** — §3.8 reframed to manufacturer-as-surface-vocabulary (Ironclad/Scrapjaw/Boltwell + wild-biome); placeholder names retired, no rename owed. Faction/part art is unblocked.
