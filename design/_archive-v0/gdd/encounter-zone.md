...
> **Exploration Progress erratum applied 2026-07-13**: Rule 8a hook wording corrected per EP Rule 2's pull-model resolution — **ZWM implements the increment; Exploration Progress persists the counter** (Rule 8a, dependency row, AC-EZ-55 activation note). No semantic change to gate math; Exploration Progress dependency row status → Approved.
> **ADR-0002 erratum applied 2026-07-13**: the gate re-evaluation trigger is now named explicitly — `EventBus.encounter_resolved(result, encounter_type)`, the Overworld Navigation relay (formerly the ambiguous 2-field "battle_ended"). TBC's 8-field combat `battle_ended` (Drop System row) is unchanged. Closes the cross-review C-1 remainder for this GDD.
...
**Evaluation timing.** The gate is (re)evaluated on `encounter_resolved` (the ADR-0002 Overworld Navigation relay — fired when the win counter may have changed and when a boss's `defeated_once` may have just flipped) and on boss-approach query — **never mid-battle**. A WILD victory that pushes `win_count` from 5→6 unlocks Boss 1 on that battle's `encounter_resolved`, not during the fight.
...
This semantic resolves the former OQ-EZ-5 as a normative rule; Zone & World Map implements the increment (advancing only on a WILD victory in the zone, per its Rules 7/8) and Exploration Progress persists the counter *(erratum 2026-07-13, per Exploration Progress Rule 2's pull-model resolution — no change to gate math)*.
...
The per-encounter transient flow (WILD): `EXPLORING` (player moving in a patch) → `ENCOUNTER_TRIGGERED` (EZ-1 roll succeeds) → enemy resolved (EZ-2) and handed to TBC → on `encounter_resolved` (ADR-0002 relay), return to `EXPLORING`. Encounter Zone holds none of this between steps — the movement/step state is Overworld Navigation's.
...
| **Exploration Progress** *(Approved 2026-07-13)* | Persistent gate state: the shared cumulative zone-win counter, the per-boss "defeated once" flag, and the per-boss `wins_at_last_defeat` snapshot — read to evaluate first-access gates (Rule 8) and the delta re-gate (Rule 9). **Storage contract fixed by Rule 8a/9**: one cumulative, never-resetting, zone-wide counter incremented only on a WILD victory (fled/lost never increment); plus, per boss, a `wins_at_last_defeat` value (re)written to the current counter on each defeat so re-access can measure the win delta. Encounter Zone owns these semantics; Zone & World Map implements the increment + snapshot at runtime (its Rules 7/8), and Exploration Progress persists the counters to this contract *(erratum 2026-07-13)*. | Approved | Soft (provisional interface; EC-EZ-11 fallback until the persistence chain is implemented) |
