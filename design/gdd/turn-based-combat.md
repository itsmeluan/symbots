...
0. **Build-validity precondition (Core Progression erratum ST-3, 2026-07-13).** Before any snapshot, TBC MUST call `CoreProgression.is_build_valid(build)` for every Symbot about to be fielded. `is_build_valid` returns false iff any equipped part's `level_requirement` exceeds the build's CORE-slot level — the post-core-swap orphaned-part case (Core Progression EC-CP-05: swapping to a lower-level CORE does not auto-unequip now-over-level parts; the build is flagged invalid instead). If **any** fielded build is invalid, **the battle does not start**: TBC refuses the encounter and returns/emits `battle_start_refused(invalid_symbot_ids, offending_parts)` for the caller (Overworld Navigation / Combat UI) to surface — no runtime state is instantiated, no turn occurs, no `battle_ended` fires. (Overworld Navigation, when authored, SHOULD additionally gate the encounter *before* handing control to TBC, so the player is warned at the Workshop rather than at the battle screen; TBC's check here is the authoritative last line of defense so an invalid build can never enter combat through any path.)
...
**Rule 12 — Battle end.** Victory: enemy `current_structure = 0` → emit `battle_ended(VICTORY, enemy_id, fired_break_events: Set, xp_value: int, completion_bonus_xp: int, is_first_boss_defeat: bool, enemy_level: int, deployed_symbot_ids: Array[int])` — break events (deduplicated set, Enemy DB ED3) for the Drop System; `xp_value`, `completion_bonus_xp` (Enemy DB field — `0` for WILD, a per-boss authored bonus for BOSS; **Core Progression erratum ST-1, 2026-07-13**), `is_first_boss_defeat` (**Core Progression refight-guard erratum, 2026-07-13** — `true` iff `enemy_class == BOSS` AND the boss's `defeated_once` was `false` at battle start; `false` for all WILD and every boss refight), and `enemy_level` consumed by Core Progression for XP awards (CP Rule 3 — the boss bonus is folded into `full_xp` **only when `is_first_boss_defeat == true`**, per CP Rule 3a's first-defeat guard, so a `LIGHTER_REGATE` boss refight awards `xp_value` alone and cannot farm the +310/+180 bonus); `deployed_symbot_ids` = the set of Symbot instance IDs fielded at any point during the battle (the deployed-vs-benched split for Core Progression Rule 3). **`is_first_boss_defeat` provenance:** the boss-approach / Overworld Navigation layer already reads the boss's `defeated_once` (owned by Zone & World Map, Rule 8) to gate the encounter; it computes this flag from that *pre-battle* value and hands it to TBC at battle start, which relays it here — passing the pre-battle value keeps the guard ordering-independent (CP need not read `defeated_once` before ZWM flips it on this same victory). Defeat: all 3 Symbots DOWNED → `battle_ended(DEFEAT, enemy_id, {}, xp_value, completion_bonus_xp, is_first_boss_defeat, enemy_level, deployed_symbot_ids)` — **no drops; Core Progression awards no XP on DEFEAT**. Fled: `battle_ended(FLED, enemy_id, {}, xp_value, completion_bonus_xp, is_first_boss_defeat, enemy_level, deployed_symbot_ids)` — no drops, no XP. The last five payload fields (`xp_value`, `completion_bonus_xp`, `is_first_boss_defeat`, `enemy_level`, `deployed_symbot_ids`) are always present for payload uniformity; Core Progression ignores them on DEFEAT/FLED outcomes. `battle_ended` is emitted **synchronously**, and all runtime combat state — current Structure/Energy/Heat, the region break pools, and the `beacon_used_this_battle` flag — is discarded **only after every subscriber has returned**: because subscribers execute inside the synchronous `emit`, the Drop System reads the still-live break set and Beacon flag before teardown. Subscriber *ordering* is deliberately **not** relied upon — Drop System and Core Progression each read disjoint runtime state or values carried in the payload, so no inter-subscriber ordering contract is required. *(Core Progression erratum 2026-07-12 — extends payload for Level Backbone; resolves OQ-TBC-6.)* **Signal-name disambiguation:** the world-progression systems (Zone & World Map, Encounter Zone) do **not** subscribe to this eight-field combat signal — **Overworld Navigation** relays a distinct `battle_ended(result, encounter_type)` to them, mapping `VICTORY/DEFEAT/FLED → WIN/LOSS/FLEE` and attaching the `WILD/BOSS` `encounter_type` it owns from the encounter trigger. The two signals share a name but are different shapes; do not wire ZWM/Encounter Zone to this combat payload. *(Cross-review erratum 2026-07-13 — C-1/S-B2.)*
...
| **Enemy AI** *(Approved 2026-07-12)* | → requests | Move choice at enemy `ACTION_PENDING`; receives visible battle state; returns exactly one legal move via `request_move(battle_state)` |
...
| **Core Progression** | `is_build_valid(build) → bool` — called at battle start (Rule 2 step 0) to refuse an invalid build before combat (EC-CP-05 / AC-TBC-42). Stateless query; TBC also *emits* to Core Progression via the Rule 12 `battle_ended` payload (mutual reference, not a design-order cycle — the query holds no state) | **Approved** *(is_build_valid consumed via ST-3 erratum 2026-07-13)* | Hard (an invalid build must never enter combat) |
...
| **Enemy AI System** | Move-choice request at enemy `ACTION_PENDING`; visible battle state | Approved (2026-07-12) | Defines 3 behavior profiles (Enemy DB ED4 discharged) returning exactly one legal move via `request_move`; respects no Heat/Energy gating (Rule 8, its AC-EAI-16). AC-TBC-INT-02 un-deferred. |
...
**AC-TBC-42** (BLOCKING): Invalid build is refused at battle start; no runtime state is created. *(Core Progression erratum ST-3, 2026-07-13 — enforces EC-CP-05's "cannot enter combat while invalid".)*
GIVEN a fielded Symbot whose build is invalid — a Boss-grade ARMS part (`level_requirement = 6`) equipped while the CORE slot holds a level-4 core (the post-core-swap orphan case), so `CoreProgression.is_build_valid(build) == false` — WHEN a battle start is requested, THEN: (a) the battle does **not** start (no `BATTLE_INIT` → `ROUND_START`, no combatant runtime state instantiated, no snapshot taken); (b) `battle_start_refused` is returned/emitted naming the invalid Symbot and the offending part(s); (c) no `battle_ended` ever fires for this non-started battle. **Positive control:** GIVEN the same roster with all builds valid (`is_build_valid == true` for all), WHEN battle start is requested, THEN the Rule 2 sequence proceeds normally. FAIL: the battle starts with an invalid build; `is_build_valid` is not consulted; runtime state is created before the check. **Test type**: Unit. *(The build-flagging/parts-listing side is Core Progression AC-CP-05; this AC verifies the combat-entry refusal that AC-CP-05 explicitly delegates here.)*
...
**AC-TBC-INT-02** (BLOCKING, Integration): Enemy AI hook returns exactly one legal move at enemy `ACTION_PENDING`; TBC resolves it through the same pipeline as player input; no energy gating applied. **Contract now defined** — Enemy AI GDD (Approved 2026-07-12) defines `request_move(battle_state) → Move`; cross-verified by Enemy AI AC-EAI-13. *Runs when: TBC's `ACTION_PENDING` state is implemented.* **Test type**: Integration.
...
| OQ-TBC-6 | ✅ **RESOLVED 2026-07-12** — Level Backbone (Core Progression #10b) adds XP as a second reward channel alongside Drop System loot. `battle_ended` extended to carry `xp_value`, `enemy_level`, and `deployed_symbot_ids` (Rule 12 erratum). Scrap salvage remains deferred to Alpha blueprint system. Drop System is the sole loot channel; Core Progression is the XP channel. | Core Progression GDD (#10b) | Closed |
