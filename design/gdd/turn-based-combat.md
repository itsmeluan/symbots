...
**Rule 12 — Battle end.** Victory: enemy `current_structure = 0` → emit `battle_ended(VICTORY, enemy_id, fired_break_events: Set, xp_value: int, completion_bonus_xp: int, enemy_level: int, deployed_symbot_ids: Array[int])` — break events (deduplicated set, Enemy DB ED3) for the Drop System; `xp_value`, `completion_bonus_xp` (Enemy DB field — `0` for WILD, a per-boss authored bonus for BOSS; **Core Progression erratum ST-1, 2026-07-13**), and `enemy_level` consumed by Core Progression for XP awards (CP Rule 3, with the boss bonus folded into `full_xp` per CP Rule 3a); `deployed_symbot_ids` = the set of Symbot instance IDs fielded at any point during the battle (the deployed-vs-benched split for Core Progression Rule 3). Defeat: all 3 Symbots DOWNED → `battle_ended(DEFEAT, enemy_id, {}, xp_value, completion_bonus_xp, enemy_level, deployed_symbot_ids)` — **no drops; Core Progression awards no XP on DEFEAT**. Fled: `battle_ended(FLED, enemy_id, {}, xp_value, completion_bonus_xp, enemy_level, deployed_symbot_ids)` — no drops, no XP. The last four payload fields (`xp_value`, `completion_bonus_xp`, `enemy_level`, `deployed_symbot_ids`) are always present for payload uniformity; Core Progression ignores them on DEFEAT/FLED outcomes. `battle_ended` is emitted **synchronously**, and all runtime combat state — current Structure/Energy/Heat, the region break pools, and the `beacon_used_this_battle` flag — is discarded **only after every subscriber has returned**: because subscribers execute inside the synchronous `emit`, the Drop System reads the still-live break set and Beacon flag before teardown. Subscriber *ordering* is deliberately **not** relied upon — Drop System and Core Progression each read disjoint runtime state or values carried in the payload, so no inter-subscriber ordering contract is required. *(Core Progression erratum 2026-07-12 — extends payload for Level Backbone; resolves OQ-TBC-6.)* **Signal-name disambiguation:** the world-progression systems (Zone & World Map, Encounter Zone) do **not** subscribe to this seven-field combat signal — **Overworld Navigation** relays a distinct `battle_ended(result, encounter_type)` to them, mapping `VICTORY/DEFEAT/FLED → WIN/LOSS/FLEE` and attaching the `WILD/BOSS` `encounter_type` it owns from the encounter trigger. The two signals share a name but are different shapes; do not wire ZWM/Encounter Zone to this combat payload. *(Cross-review erratum 2026-07-13 — C-1/S-B2.)*
