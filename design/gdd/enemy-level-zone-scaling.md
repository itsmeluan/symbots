...
| **Symbot Core Progression** (downstream) | → supplies + calibrates | `enemy.level` is the `enemy_level` input to CP-F4; `enemy.xp_value` + `enemy.completion_bonus_xp` are passed in the `battle_ended` payload per the TBC erratum. This system resolves OQ-CP-1 (MVP zone level range = [1, 6]), OQ-CP-2 (label model), and **OQ-CP-8** (equip-gate coverage — calibrates the per-boss `completion_bonus_xp`, Boss 1 = 310 / Boss 2 = 180, against AC-ELZS-14). |
...
|--------|------------------------------|-----------|--------|
| **Enemy Database** (#2) | Adds `level: int` and `xp_value: int` to the enemy entry schema. Reads existing `enemy_class` (WILD/BOSS) as the `role_multiplier` input to CP-F4. | **Hard** — without `level`, neither XP nor drop rarity scaling can function | Approved ✓ *(erratum: add `level` + `xp_value` fields and stored-equals-derived validation)*. **Integration contract:** this system's label/anchor model (Rule 1) depends on AC-ED-14 (EDB-2 TTK calibration) to enforce level-vs-stat consistency — a level-4 enemy authored with level-1 stats passes every ELZS content validation AC while betraying the Player Fantasy; AC-ED-14 is the only cross-system guard. If AC-ED-14 is weakened or removed, note the dependency. |
...
- **Enemy Database erratum**: Add `level: int` and `xp_value: int` to schema; stored-equals-derived validation (AC-ELZS-02 pattern); add "depended on by Enemy Level & Zone Scaling" to dependencies section. **(Core Progression ST-1, 2026-07-13):** also add `completion_bonus_xp: int` (0 for WILD; per-boss for BOSS — Boss 1 = 310, Boss 2 = 180), calibrated here against the equip-gate coverage AC-ELZS-14; content validation `≥ 0` and `0` unless `enemy_class == BOSS`.
...
**AC-ELZS-14** (BLOCKING) — Equip-gate XP coverage: worst-case floor path clears each rarity gate (resolves Core Progression OQ-CP-8). **GIVEN** the MVP zone (`enemy_level_floor = 1`, so the floor WILD XP is CP-F4(WILD, L1) = `(35 + 1×10)×1 = 45`), the WILD-only boss win gates (Rule 8a: Boss 1 `required_wins = 6`; Boss 2 `required_wins = 10` **and** `requires_defeated = Boss 1` per Encounter Zone Rule 8, so Boss 1 is always defeated before Boss 2), the boss awards (`xp_value` + `completion_bonus_xp`: Boss 1 = 170 + **310**; Boss 2 = 190 + **180**), and the CP-F1 thresholds (`threshold[6] = 744`, `threshold[8] = 1292`), **WHEN** the coverage expression is evaluated for each rarity equip gate, **THEN** the worst-case floor-level approach path clears the gate the boss's drop requires:
- **Boss-grade gate (level 6):** `(6 × 45) + (170 + 310) = 270 + 480 = 750 ≥ 744` ✓
- **Prototype gate (level 8):** `(10 × 45) + (170 + 310) + (190 + 180) = 450 + 480 + 370 = 1300 ≥ 1292` ✓

Coverage expression: `(required_wins_to_gate × xp_value[enemy_level_floor]) + Σ(xp_value + completion_bonus_xp over every boss defeated by that gate) ≥ threshold[gate_level]`. **First-defeat basis:** the `completion_bonus_xp` terms count each boss's **first** defeat (the equip-gate approach path is a first-clear), which is exactly when the bonus is awarded — CP Rule 3a's first-defeat guard (`is_first_boss_defeat`) suppresses the bonus on refights (AC-CP-25), so this coverage math is unaffected by refights and the bonus cannot be farmed to trivialize later gates. **Discriminators:** (a) the expression MUST use `enemy_level_floor` (45), not `enemy_level_roof` (L6 → 85) — an implementation using the roof reports large false surpluses and masks the real early-terrain shortfall this AC exists to catch; (b) `required_wins` is **WILD-only** (boss wins do not increment `win_count` — ZWM EC-ZWM-07), so Boss 2's path is a full 10 WILD plus the guaranteed Boss 1 award, not 9. If any future retune of `completion_bonus_xp`, CP-F1 (`XP_PER_LEVEL_BASE`/`LEVEL_COST_RAMP`), CP-F4 (`XP_BASE`/`XP_PER_ENEMY_LEVEL`), or the `required_wins`/rarity-gate-floor tables makes any gate's floor path fall below its threshold, this AC **fails (BLOCKING)** — the correct fix is a **per-gate** lever (that boss's `completion_bonus_xp`, its `required_wins`, or the rarity `level_requirement` floor), never a blanket `BOSS_XP_MULTIPLIER`/`XP_PER_ENEMY_LEVEL` change (which scales both gaps together and over/undershoots one — Core Progression OQ-CP-8 single-lever caution). *(Core Progression Rule 3a + OQ-CP-8; Enemy DB `completion_bonus_xp`; CP-F1/CP-F4; Encounter Zone Rule 8/8a)* **Test:** Content Validation / calculation check.

**Summary: 11 BLOCKING (including AC-ELZS-11 integration gate + AC-ELZS-14 equip-gate coverage) + 1 ADVISORY + 2 delegated citations.**

> **Errata pre-gate (process requirement — ✅ ALL FOUR ERRATA APPLIED 2026-07-13):** Four Approved GDDs receive amendments from this system. Before any implementation story for this GDD begins, confirm all four errata are applied: (1) Enemy DB — `level: int`, `xp_value: int` fields + stored-equals-derived validation; (2) Encounter Zone — `enemy_level_floor: int`, `enemy_level_roof: int` + in-band validation; (3) Drop System — DS-F-LEVEL factor + LEVEL_RARITY_MULTS table + economy-model Rare-throughput re-annotation (see Bidirectionality Notes), **plus three erratum-integrity obligations**: (3a) replace the Drop System's partially overlapping DS-1 expressions (Rule 4 base form; Rule 12a Beacon note) with **one canonical amended DS-1** — `clamp(base_drop_rate[rarity(p)] × level_rarity_mult × Π(condition_multipliers) × beacon_factor, 0, 1)` — retiring or "as amended"-labeling the partial forms; (3b) update **AC-DS-31** (Beacon fixture), which predates DS-F-LEVEL and would coincidentally pass an implementation that never wires `level_rarity_mult` — add a stated enemy level, e.g. L6/HIGH, no conditions, Beacon → `clamp(0.25 × 1.5 × 2.0) = 0.75`, draw 0.60 drops (an implementation missing the level factor returns 0.50 and does not drop); (3c) document the production `effective_drop_rate()` interface and land AC-ELZS-11's integration test as a **Done condition of the erratum story** (see AC-ELZS-11); (4) Zone & World Map — Rule 4 guideline table + ADVISORY validation. Sprint task for each errata; AC-ELZS-11 serves as the integration gate that confirms Errata 3 is live in code.
>
