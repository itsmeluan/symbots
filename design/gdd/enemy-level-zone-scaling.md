...
- **Drop System erratum**: Amend DS-1 expression to include `level_rarity_mult` factor; add LEVEL_RARITY_MULTS table to Tuning Knobs; add DS-F-LEVEL to Formulas; add "depended on by Enemy Level & Zone Scaling" to dependencies section.
...
| `LEVEL_RARITY_MULTS[HIGH][Rare]` | float | 1.5 | This system | Rare drop multiplier in the HIGH band. **Warning**: values ≥ 2.0 mean Rare + Beacon + any condition → `0.25 × 2.0 × cond × 2.0 ≥ 1.0` — guaranteed drop without break execution. Safe range 1.2–2.0. |
...
- **Combat UI (#19):** enemy `level` displayed at encounter start and on the battle HUD ("Lv 4 Crawler"). The level tag is the player's XP/drop-band signal.
...
**EC↔AC Cross-Check:** EC-ELZS-01 → AC-01 · EC-02 → AC-01 · EC-03 → AC-05 · EC-04 → AC-02 · EC-05 → AC-03 · EC-06 → AC-05 · EC-07 → AC-10 (expected behavior) · EC-08 → AC-CP-08 (delegated) · EC-09 → AC-06 · EC-10 → AC-09 · EC-11 → AC-04. **All 11 ECs covered.**

**Summary: 7 BLOCKING + 1 ADVISORY + 2 delegated citations.**
