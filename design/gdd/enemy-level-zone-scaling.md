...
Only Rare is scaled. Common stays reliable for floor loot at all tiers. Boss-grade is already gated by ×500 break multipliers. Prototype stays at 0.05 across all bands.
...
- **Drop System erratum**: Amend DS-1 expression to include `level_rarity_mult` factor; add LEVEL_RARITY_MULTS table to Tuning Knobs; add DS-F-LEVEL to Formulas; add "depended on by Enemy Level & Zone Scaling" to dependencies section. **Economy model re-annotation (mandatory):** DS-F-LEVEL reduces the arc-average Rare drop rate below the ~0.36/victory baseline the Drop System's `≈1,840 Scrap central` model assumes (that model was derived before DS-F-LEVEL existed). Specifically, the EARLY band's ×0.5 Rare mult depresses hours-1–3 Rare throughput; the single-zone MVP's [1,6] band mix (predominantly MID, with EARLY at L1-2 and HIGH at L6 only) lowers the arc-average from ~0.36 to ~0.27 Rares/victory (~54 Rares vs. 72, ~540 Rare Scrap vs. 720, revised central ~1,660 vs. 1,840). Annotate the Drop System's economy model with this correction; the Scrap model's "mild scarcity" band validity should be re-confirmed at the revised figure.
...
ACs marked **BLOCKING** gate story completion. Content Validation ACs run at content-authoring time (schema/data validator); Unit ACs are automated tests. The two Unit ACs (09, 10) belong in `tests/unit/drop_system/` — they test the same `level_band()` / `effective_drop_rate()` functions that DS-F-LEVEL amends.
...
**AC-ELZS-02** (BLOCKING) — `xp_value` stored equals CP-F4 derived. **GIVEN** any Enemy Database entry with authored `level` and `enemy_class` (WILD or BOSS), **WHEN** the content validator runs, **THEN** `stored_xp_value == (XP_BASE + level × XP_PER_ENEMY_LEVEL) × role_multiplier` where `XP_BASE = 35`, `XP_PER_ENEMY_LEVEL = 10`, WILD `role_multiplier = 1`, BOSS `role_multiplier = 2`. Concrete checks: WILD level 1 → `xp_value == 45`; WILD level 3 → `xp_value == 65`; BOSS level 5 → `xp_value == 170`; BOSS level 6 → `xp_value == 190`. An entry storing any deviating value — including a WILD level-3 entry storing a stale `70` from a pre-retune derivation — fails (BLOCKING). *(Rule 2; EC-ELZS-04)* **Test:** Content Validation.
...
**AC-ELZS-04** (BLOCKING) — Zone band fields present and in global range. **GIVEN** any Encounter Zone entry, **WHEN** the content validator runs, **THEN** both `enemy_level_floor` and `enemy_level_roof` are present integer fields, `enemy_level_floor >= 1`, and `enemy_level_roof <= MAX_ENEMY_LEVEL (10)`. Missing either field fails (BLOCKING); `enemy_level_roof = 11` fails. The MVP zone definition (`floor = 1, roof = 6`) must pass. *(Rule 3; EC-ELZS-11)* **Test:** Content Validation.

**AC-ELZS-05** (BLOCKING) — All spawn-pool enemies within the zone's level band. **GIVEN** an Encounter Zone entry with `enemy_level_floor = F`, `enemy_level_roof = R`, and a spawn pool referencing enemy entries by ID, **WHEN** the content validator resolves each referenced enemy's `level` and checks membership, **THEN** every enemy `level ∈ [F, R]` (inclusive both ends). **(A)** zone `[1, 6]`, pool levels `[1, 3, 5, 6]` → passes. **(B)** zone `[3, 6]`, pool includes a level-2 enemy → fails (BLOCKING), identifying the out-of-band enemy — an implementation checking `level > F − 1` rather than `level >= F` incorrectly accepts this case at non-integer boundaries and is caught by the explicit fixture. **(C)** zone `[1, 6]`, pool includes a level-7 enemy → fails. **(D)** empty pool with valid band → fails (BLOCKING) per EC-ELZS-12 — a zone with no enemy entries cannot run encounters. Note: this is distinct from EC-ELZS-06 (non-empty pool, all out-of-band); an implementation that only checks in-band membership and skips the empty-pool guard passes fixtures A–C but fails D. *(Rule 3; EC-ELZS-03; EC-ELZS-06; EC-ELZS-12)* **Test:** Content Validation.

**AC-ELZS-06** (ADVISORY) — `difficulty_band` label consistent with `enemy_level_floor`. **GIVEN** any zone entry with a `difficulty_band` value and an authored `enemy_level_floor`, **WHEN** the content validator runs, **THEN** if the floor falls outside the Rule 4 guideline range for that band — EARLY: floor ∈ [1, 3]; MID: [3, 6]; LATE: [6, 9]; ENDGAME: [8, 10] — the validator emits an ADVISORY warning naming the inconsistency. `EARLY + floor = 7` warns; `EARLY + floor = 1` is silent. The MVP zone (`floor = 1, band = EARLY`) must emit no warning. Does not block content from shipping. *(Rule 4; EC-ELZS-09)* **Test:** Content Validation.
...
**AC-ELZS-09** (BLOCKING) — `level_band()` boundary discriminators. **GIVEN** the `level_band(level)` sub-function (DS-F-LEVEL), **THEN**: `level_band(3) == MID` (the discriminating input — a `>`-based implementation misclassifies L3 as EARLY, halving its Rare multiplier from 1.0 to 0.5); `level_band(2) == EARLY`; `level_band(6) == HIGH`; `level_band(5) == MID`. *(Rule 5; EC-ELZS-10)* **Test:** Unit.
...
**AC-ELZS-11** (BLOCKING) — DS-F-LEVEL amendment live in production Drop System. **GIVEN** the production `effective_drop_rate()` implementation in the Drop System (not a standalone test stub), **WHEN** called with a level-2 enemy, a Rare part, one fired condition at ×1.5, and `beacon_factor = 1.0`, **THEN** the function returns **0.1875** — not 0.375. A return of 0.375 means `level_rarity_mult` was not wired (EARLY mult 0.5 not applied); a return of 0.1875 confirms the amendment is live. *(Rule 5; DS-F-LEVEL; Bidirectionality Notes)* **Test:** Integration (post-Drop System erratum implementation).

**EC↔AC Cross-Check:** EC-ELZS-01 → AC-01 · EC-02 → AC-01 · EC-03 → AC-05 · EC-04 → AC-02 · EC-05 → AC-03 · EC-06 → AC-05 · EC-07 → AC-10 (expected behavior) · EC-08 → AC-CP-08 (delegated) · EC-09 → AC-06 · EC-10 → AC-09 · EC-11 → AC-04 · EC-12 → AC-05(D). **All 12 ECs covered.**

**Summary: 8 BLOCKING (including AC-ELZS-11 integration gate) + 1 ADVISORY + 2 delegated citations.**

> **Errata pre-gate (process requirement):** Four Approved GDDs receive amendments from this system. Before any implementation story for this GDD begins, confirm all four errata are applied: (1) Enemy DB — `level: int`, `xp_value: int` fields + stored-equals-derived validation; (2) Encounter Zone — `enemy_level_floor: int`, `enemy_level_roof: int` + in-band validation; (3) Drop System — DS-F-LEVEL factor + LEVEL_RARITY_MULTS table + economy-model Rare-throughput re-annotation (see Bidirectionality Notes); (4) Zone & World Map — Rule 4 guideline table + ADVISORY validation. Sprint task for each errata; AC-ELZS-11 serves as the integration gate that confirms Errata 3 is live in code.

## Open Questions

- **OQ-ELZS-1 — Alpha zone band authoring.** Rule 4's LATE/ENDGAME rows are guidelines with no content behind them until Alpha zones are designed. Re-validate the difficulty_band mapping and the LEVEL_RARITY_MULTS HIGH values when the second zone (Vertical Slice) is authored — the ×1.5 Rare mult calibration assumes MVP's single-zone context. *Owner: Vertical Slice zone design pass.*
