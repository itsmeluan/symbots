...
| `chassis_archetype` | Enum | Chassis archetype defining stat multipliers: one of `LIGHT_FRAME, HEAVY_FRAME, BALANCED_FRAME, GUARDIAN_FRAME, ARTILLERY_FRAME`. Unset (enum sentinel `0`) for all non-CHASSIS slot types — see the typed-export sentinel convention below this table. Required when `slot_type = CHASSIS`; must be unset otherwise. See Rule 3 and Formula 1. |
...
| `stat_bonuses` | Dictionary[StringName, int] | Stat name → integer bonus (e.g., `{ &"structure": 40, &"armor": 12 }`). Key = canonical stat name as **`StringName`** — the formula pipeline reads stat keys as `StringName` literals (`&"structure"`); typed-Dictionary lookups do not coerce `String`↔`StringName` in Godot 4.7, so a `String`-keyed authored dict would make every stat lookup silently return 0. Same hazard class as `level_growth` (key type pinned there in Round 10); pinned here in Round 11 — godot-specialist finding. Formula pseudocode in this document writes `stat_bonuses["armor"]` for readability; it always denotes the `StringName` key `&"armor"`. |
...
| `level_requirement` | int | Core level required to equip this part. Authoring floors by rarity (CP Rule 5): COMMON=1, RARE=3, BOSS_GRADE=6, PROTOTYPE=8. Individual parts may have a higher `level_requirement` than their rarity floor; never lower. `0` (the typed-int default — typed exports cannot hold `null`; see the sentinel convention below this table) defaults to no gate (treated as 1). *(Core Progression erratum 2026-07-12.)* |
...
> **Typed-export sentinel convention (Round 11, 2026-07-16 — godot-specialist):** GDScript typed `@export` fields cannot hold `null`. Wherever this document says "`null`" for a typed field, content and code use the type's empty sentinel: `&""` for `StringName` fields (`active_skill_id`, `passive_id`, `part_family`, `sprite_id`), `0` for int/enum fields (`chassis_archetype` reserves enum value 0 as unset; `level_requirement` 0 = no gate), and `{}` / `[]` for Dictionary/Array fields. Validators and ACs that test "null" / "non-null" (e.g., AC-01, AC-14, AC-20) are testing against these sentinels.
...
| Prototype | 0.05 | Gradient conditional (see Rule 8): optimal play — all conditions fired — reaches ~16.9–20%; partial fire yields a partial rate. **Content rule:** every Prototype must define ≥3 drop conditions whose full multiplier product is ≥ ×3.0 (e.g., three ×1.5 conditions: 0.05 × 3.375 = 0.169), otherwise the ~16.9–20% optimal-play target is unreachable. *(Round 11, economy-designer: band floor corrected from "15%" — minimum-compliant content lands at 16.875%, so 15–16.8% is unreachable under compliant authoring.)* |
...
- 3 fired: 0.05 × 1.5³ = **0.16875** (~17% — the floor of the ~16.9–20% optimal-play target band)
...
**Prototype focus-stat floor (Round 10, 2026-07-16; amended Round 11):** every Prototype's focus stat **is its slot's primary stat** (slot primary-stat mapping below; Arms/Weapon resolve per-part by `damage_type`) and must be **strictly greater than the slot's Rare primary FLOOR** (table below) — otherwise a Prototype at +0 can read as a pure downgrade against a Rare in the same slot, betraying EC-10's acquisition intent. *(Round 11: the Round 10 form — "highest positive bonus, whichever key it is" — let a Chassis Prototype focus Armor while its Structure sat at 10, a pure primary-stat downgrade that still passed. Off-primary-focus Prototypes are not authorable in MVP.)* This is why the Chassis Prototype minimum budget was raised 40 → 42: at the old minimum, a guideline-following author (70% of 40 = 28) landed **below** the Rare Chassis floor of 29 — the single off-by-one slot in the table. **General authoring rule (Round 11 — not Chassis-specific):** the naive "70% of minimum budget" allocation is not guaranteed to clear the floor — for any slot where `floor(0.70 × min_prototype_budget) <= rare_primary_floor`, the focus stat must be authored strictly above the floor even where that exceeds 70% (Chassis: `floor(0.70 × 42) = 29`, not > 29, so ≥ 30 is required — ~71.4% of 42, legal because the 70%+ concentration rule is a floor on concentration, not a target). *Verified by AC-25.*
...
**AC-25** *(Round 10, 2026-07-16; amended Round 11, 2026-07-16)*: Every Prototype part's focus stat **is its slot's primary stat**, and at +0 it strictly exceeds the slot's Rare primary FLOOR (Stat Budget Reference tables). **Pass when**: For every `PROTOTYPE` entry, resolving `primary_stat` via the slot primary-stat mapping (Arms/Weapon by the part's `damage_type`, exactly as in AC-23): (a) `stat_bonuses[primary_stat]` is the part's highest positive stat bonus — no other stat strictly exceeds it (ties with a secondary stat are permitted) — zero violations; (b) `stat_bonuses[primary_stat] > rare_primary_floor[slot]` — zero violations. *(Round 11 amendment — closes the any-key loophole: the Round 10 wording accepted the highest positive bonus "whichever stat key it is", so a Chassis Prototype authored `{structure: 10, armor: 30, mobility: -8}` passed (30 > 29) while being a pure downgrade in Structure — the exact failure EC-10 exists to prevent. Off-primary-focus Prototypes are **not authorable** in MVP: the primary IS the focus. Design note: any keep-both-rules alternative — off-primary focus plus a primary-stat minimum — is budget-infeasible anyway under AC-19's 70% concentration [an off-primary Chassis Prototype would need focus ≥ 45 plus primary ≥ 19 → total ≥ 64 > the 55 budget maximum]; pinning focus = primary makes the constraint checkable instead of emergent.)* Boundary: a Chassis Prototype with `structure = 29` **fails** (b) (29 is not > 29 — the exact off-by-one that motivated the AC); `structure = 30` passes. Discriminating fixtures: (i) Chassis Prototype `{structure: 10, armor: 30, mobility: -8}` must **FAIL** (a) — a validator that only checks "max positive bonus > floor" passes this wrong implementation; (ii) Chassis Prototype at minimum budget (42) with `structure = 29` must **FAIL** (b) — a validator comparing `>=` instead of `>` passes the wrong implementation. Enforces EC-10's acquisition intent, which was previously an unenforced prose instruction. **Test type**: Content Validation.

**AC-26** *(Round 10, 2026-07-16)*: Every Prototype part satisfies the Formula 3 content rule — ≥3 drop conditions with sufficient combined strength. **Pass when**: For every `PROTOTYPE` entry: (a) `drop_conditions.size() >= 3` — zero violations; (b) `product(entry.multiplier for entry in drop_conditions) >= 3.0` — zero violations. (All authored multipliers are already > 1.0 per Rule 9, so the full product is the favorable product. *Round 11:* that `> 1.0` invariant is owned and validated by Rule 9 / Drop System Rule 5a — AC-26 depends on it but does not re-check it; if penalty multipliers are ever reintroduced, AC-26's full-product assumption must be revisited. The product is computed over **all authored** `drop_conditions` entries — AC-26 is a static content check; runtime condition matching belongs to Formula 3, not this AC.) Boundary: three conditions at ×1.5 each → product 3.375, passes; three conditions at ×1.4, ×1.4, ×1.5 → product 2.94, **fails** (b); two conditions at ×2.0 each → product 4.0 but **fails** (a) — both sub-checks are independently required. Rationale: a Prototype below this floor makes the ~16.9–20% optimal-play target unreachable and silently unhinges the Drop System's pity calibration (`N_PROTO_PITY = 25` assumes optimal rate ≥ 0.16875; at product ×2.0 the pity path becomes ~7% expected instead of ~1%). This was a stated cross-system contract with no enforcement — flagged independently by three reviewers. **Test type**: Content Validation.
