...
| `&"world_loot"` | World Loot System *(provisional — #13 not yet authored)* | Flat `Set[StringName]` of collected `loot_id`s. **Serialized form (Godot 4.6 has no native Set):** runtime form is `Dictionary[StringName, bool]` (the Godot set idiom); `snapshot()` returns a **sorted** `Array[StringName]` — sorted via **String-cast comparison** (`sort_custom` on `String(a) < String(b)`; raw `StringName` `operator<` compares session-unstable intern indices and is forbidden — a raw sort produces non-deterministic output); `restore()` accepts an Array and dedupes on Set reconstruction. |
