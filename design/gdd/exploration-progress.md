...
| `&"world_loot"` | World Loot System *(provisional — #13 not yet authored)* | Flat `Set[StringName]` of collected `loot_id`s. **Serialized form (Godot 4.6 has no native Set):** runtime form is `Dictionary[StringName, bool]` (the Godot set idiom); `snapshot()` returns a **sorted** `Array[StringName]` (deterministic output); `restore()` accepts an Array and dedupes on Set reconstruction. |

Reserved, not MVP: `&"key_items"` (#23a, Vertical Slice). Domain keys are **save-format-stable — never rename one**.
...
**Rule 8 — The Exploration Progress ↔ Save/Load split (provisional until #17).** This system produces and consumes the **progression blob** (one Dictionary). Save/Load owns everything below it: file format/encoding, save timing and triggers, slots, disk I/O, file-level corruption handling — and directly serializes the *non-progression* state contracted to it elsewhere (Inventory including `next_instance_id` + Scrap, Workshop builds, Drop System pity maps, Settings). Those do not pass through this system.

**Rule 9 — Format version.** The blob carries `progress_format_version = 1`. On load: same version → restore; older → per-domain migrate hook (reserved, none needed at v1); newer → refuse the blob and surface to Save/Load's error path (never partially restore a future format). **A missing or non-int `progress_format_version` key → REFUSE** (unknown format — never guess a version). **REFUSE guarantee:** a refused blob leaves all domain in-memory state unchanged from before `restore()` was called — no domain receives any data, no partial restore, ever.
...
| **World Loot** (#13, Not Started) | domain (`&"world_loot"`) — *provisional* | WL owns the runtime collected set + globally-unique `loot_id` content validation; implements the Rule 3 contract. This row is the contract #13 must satisfy. |
| **Encounter Zone** (Approved) | indirect | EZ never reads this system — its gate-check receives `win_count`/`boss_progress` from ZWM (ZWM Rule 8). This system's shipping completes the persistence chain, activating deferred **AC-EZ-40b** and **AC-EZ-55**. **EZ erratum owed**: Rule 8a hook wording (see Rule 2). |
...
result =
  ...
  elif saved_version < CURRENT_FORMAT_VERSION: MIGRATE   # per-domain hook reserved; none at v1
...
**Worked examples:** saved=1 / current=1 → RESTORE · saved=0 → MIGRATE (hook reserved) · saved=2 → REFUSE · key missing → REFUSE.
...
**On violation:** clamp per Rule 6(e), log a corruption warning, continue restore. **Rationale:** a `wins_at_last_defeat > win_count` entry would feed a **negative delta** into Encounter Zone's LIGHTER_REGATE check (`win_count − wins_at_last_defeat >= regate_params.required_wins`), which is unspecified for negatives. Clamping `wins_at_last_defeat` to `win_count` yields delta = 0: the re-gate then requires a full `regate_params.required_wins` fresh wins — the most conservative recovery (re-gate behaves as if the boss was just defeated at the current count). Never pass invalid values into ZWM. Pure integer comparison — **scan-exempt.**
...
**EC-EP-03 — `wins_at_last_defeat > win_count` (tampered/corrupt save).** *If* a restored boss entry violates EP-INV-1: clamp `wins_at_last_defeat = win_count` + corruption warning (Rule 6e). Re-gate delta becomes 0 — the most conservative recovery. Never passed raw into ZWM. *Verified by AC-EP-05.*
...
| **Encounter Zone** (#7) | Nothing read at runtime — but EZ Rules 8a/9 *fix the semantics* of the fields this system persists (`win_count` wins-only/never-resets; `wins_at_last_defeat` snapshot). EP-INV-1 exists to protect EZ's delta math. | **Soft** (semantic dependency) | Approved ✓ *(erratum owed: Rule 8a hook wording → "ZWM implements the increment; Exploration Progress persists the counter" — see Rule 2)* |
| **World Loot** (#13) | Future domain owner for `&"world_loot"` — must implement the Rule 3 contract and globally-unique `loot_id` content validation. | **Soft-provisional** — until authored, the domain is simply unregistered; nothing breaks | Not Started *(Rule 1 row is the contract #13 must satisfy)* |

### Downstream Dependents (what depends on this system)

| System | What it reads | Status |
|---|---|---|
| **Save/Load** (#17) | The progression blob (Rule 8 split: this system owns blob semantics; Save/Load owns file format, timing, slots, disk I/O). Must list this system when authored. | Not Started |
...
- **Encounter Zone erratum (light):** reword Rule 8a's "Exploration Progress must implement its increment hook" per Rule 2's resolution. One line; no semantic change to EZ's gate math.
...
**AC-EP-08** (BLOCKING, Unit) — **Duplicates: loot dedupe + cores first-wins (synthetic IDs).** **Part A:** saved loot Array `["chest_synth_1", "chest_synth_2", "chest_synth_1", "chest_synth_2", "chest_synth_3"]` (5 entries, 3 unique) → runtime set size 3; snapshot returns sorted 3-element Array; malformed-data warning. **Part B:** duplicate `core_instance_id` collision — **fixture technique note:** both GDScript Dictionary literals *and* Godot's JSON parser collapse duplicate keys (last-wins) before restore code runs, so the collision must be **injected directly at the domain-restore API level using an Array-of-records form** (`[{id: "cx-dupe-A", cumulative_xp: 364}, {id: "cx-dupe-A", cumulative_xp: 993}]`); **THEN** `cx-dupe-A` present exactly once with `cumulative_xp == 364` (**first** occurrence wins — a last-wins impl stores 993 and fails), re-derived level 4, warning logged. *(Rule 6e; EC-EP-07; EC-EP-16)*
