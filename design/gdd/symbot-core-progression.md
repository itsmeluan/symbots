...
> **Status**: In Design (re-review 2026-07-13 — C-2 + D-2 confirmed RESOLVED; NEEDS REVISION on 5 new hygiene blockers, in-doc fixes applied. **In-doc DONE:** logging-spy interface (Rule 1), AC-CP-22 cooling portion demoted to ADVISORY-until-established, AC-CP-21 discriminating fixture corrected, OQ-CP-6 CD ratification recorded, OQ-CP-8 re-derived + orphan flagged, AC-CP-18 traceability requirement strengthened. **Cross-doc obligations still owed (producer-coordinated, not in-doc):** (1) ELZS erratum absorbing the OQ-CP-8 Pillar-2 equip-gate AC; (2) Assembly erratum AC gating AC-CP-18; (3) Part DB/SA-F1 `cooling` output range to promote AC-CP-22 cooling check to BLOCKING.)
...
**Rule 2 — Level derivation.** Level is a deterministic function of `cumulative_xp`, computed via CP-F1 (the XP threshold table). The table is derived from a base cost of `XP_PER_LEVEL_BASE` with a `LEVEL_COST_RAMP` multiplier per level (see Formulas). Level never decreases. After any XP gain, re-derive level and emit `core_leveled_up(core_instance_id, old_level, new_level)` if level increased. Multiple levels in a single XP gain are possible — in this case, emit **once** with the final span (`old_level → new_level`); consumers that unlock per-level content iterate the crossed range.
...
- **OQ-CP-8 — Equip-gate frustration (Pillar-2 calibration). ⚠️ ORPHANED OBLIGATION — needs a tracked AC in ELZS (Approved).** With the now-**confirmed** CP-F4 constants (`XP_BASE = 35`, `XP_PER_ENEMY_LEVEL = 10`, `BOSS_XP_MULTIPLIER = 2`) and the confirmed level-1–6 MVP zone, the gap is real, not provisional. **Re-derived with confirmed numbers (2026-07-13):**
