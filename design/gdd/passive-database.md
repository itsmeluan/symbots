...
> **Status**: Approved — 2026-07-10 (re-review round 2 passed; 5 blocking prose fixes applied and accepted)
...
- **STRUCTURAL_EFFECT numeric values** — same pattern: per-entry authored integers, clamped by TBC's Structure floor (0) and ceiling (current `max_structure` at the moment of trigger). **Content constraint:** a `STRUCTURAL_EFFECT` `amount` must be **non-negative for both targets** (`CURRENT_STRUCTURE` and `MAX_STRUCTURE`). A negative `CURRENT_STRUCTURE` amount means self-damage at trigger time; a negative `MAX_STRUCTURE` amount means a max-HP debuff — and unlike `STAT_AURA` (where a negative delta is a legitimate persistent debuff), STRUCTURAL_EFFECT is reserved for event-triggered structure *gain* (heal/repair). Persistent structure debuffs belong on a negative `STAT_AURA` over the `structure` stat instead. Content validation rejects negative amounts of either target (AC-PDB-16); at runtime the floor clamp prevents a crash but the effect is a design error (EC-PDB-08). *(Future extension: a mid-battle, event-triggered `MAX_STRUCTURE` reduction — e.g., an Overheat-spiral drawback — is a deliberate non-goal for MVP. Reopening the negative-amount door is a non-breaking widening; do it only when content demands it.)*
...
**EC-PDB-08 — `STRUCTURAL_EFFECT` passive authored with a negative `amount` (either target).** A content authoring error (e.g., `amount: -20` instead of `20`) that would damage the carrying Symbot (`CURRENT_STRUCTURE`) or shave its max HP (`MAX_STRUCTURE`) when the passive triggers. Content validation rejects it at authoring time (naming the passive ID). At runtime, if it ships anyway: TBC applies it through the Structure floor clamp — `current_structure = max(0, current_structure + amount)` — so Structure cannot go below 0 and no crash occurs, but the Symbot takes unintended self-damage. This mirrors EC-PDB-05's treatment of negative `STAT_AURA`, except a negative structural amount is *never* legitimate — persistent structure debuffs go through a negative `STAT_AURA` on the `structure` stat (EC-PDB-05), which is the sanctioned path. *Verified by AC-PDB-16 (content validator) and AC-PDB-17 (runtime clamp).*
...
**AC-PDB-09** (BLOCKING): a `UNIQUE` passive granted twice (part `passive_id` + a second source) applies its effect **exactly once** — the second source adds no additional effect. Observable via the effect's magnitude, not internal registry inspection: GIVEN a `UNIQUE` `STAT_AURA` with a known `delta` granted by two sources on the same Symbot, WHEN the aura applies at BATTLE_INIT, THEN the affected stat shifts by exactly **one** `delta`. FAIL: the stat shifts by 2× `delta` (both instances applied); the stat is unchanged (neither applied).
...
**AC-PDB-16** (ADVISORY-DEFERRED): a passive entry whose `behavior_params` does not match its `behavior_class` (missing a required key, wrong key set, or a negative `STRUCTURAL_EFFECT` `amount` for either target per EC-PDB-08) — content validator rejects it naming the passive ID and the offending field. *Unblocks when: schema validation tooling exists. Verifies EC-PDB-08 (authoring path).*
...
**21 ACs total (17 live + 4 deferred):** 12 BLOCKING unit (AC-PDB-01–11, 17) + 5 ADVISORY-DEFERRED content (AC-PDB-12–16) + 4 activates-on-first-content (AC-PDB-D1–D4, become BLOCKING when OQ-PDB-1 authors the matching path).
