...
| 10c | Enemy Level & Zone Scaling | World | MVP | In Review (2026-07-12, full-panel /design-review — NEEDS REVISION → 7 fixes applied same session: EC-ELZS-12 empty-pool, AC-ELZS-11 DS-1 integration gate, errata pre-gate block, Drop System economy erratum note, Tuning Knobs warning corrected, Boss 1 MID-band rationale, UI drop-band legibility requirement. Re-review pending fresh session. 4 errata still owed on approval: Enemy DB/Encounter Zone/Drop+economy-model/ZWM) | design/gdd/enemy-level-zone-scaling.md | Enemy Database, Encounter Zone, Drop System, Zone & World Map |
