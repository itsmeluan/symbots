...
|---|-------------|----------|----------|--------|------------|------------|
| 1 | Part Database | Foundation | MVP | **Approved** (2026-07-16, Round 11 full-panel /design-review [5 specialists + CD] re-reviewing the 7 Round-10 fixes — NEEDS REVISION: 3 new blockers [AC-25 any-key focus loophole: `{structure:10, armor:30}` Chassis Prototype passed while a pure primary-stat downgrade; AC-08(b) base −1 fixture non-discriminating vs a no-reduction implementation; `stat_bonuses` missing `Dictionary[StringName, int]` pin — String keys silently read 0]. All 3 + 7 recommended fixed same session. **User design decision: Prototype focus stat = slot primary stat** — off-primary Prototypes are not authorable in MVP. CD targeted fix-confirmation APPROVED, no re-panel. **Production debt OPEN (escalated to producer):** Round-10 validator-hardening fast-follow story was never created; AC-25/26/27 have zero validator implementation; formula ACs 05–09/16 have no on-disk tests — the Round-10 log's "271/271, ContentValidator enforces every AC" claim overstates reality. Prior Round 10: NEEDS REVISION, 7 blockers, fixed same day) | design/gdd/part-database.md | — |
...
| 6 | Turn-Based Combat System | Gameplay | MVP | Approved (2026-07-13, cross-review erratum applied — battle_ended synchronous-teardown/ordering contract + ZWM/EZ relay signal-name disambiguation, C-1/S-B2) | design/gdd/turn-based-combat.md | Damage Formula, Symbot Assembly, Enemy Database, Synergy, Part Database, Part-Break |
...
| 10b | Symbot Core Progression (Leveling) | Gameplay | MVP | **Approved** (2026-07-13, 4th-pass full-panel confirmation re-review + all errata discharged + user acceptance. C-2/D-2 re-confirmed RESOLVED; all 4 formulas re-verified; CD: design is sound. 2 in-doc fixes (AC-CP-21 float-cast; Rule 2/EC-CP-02 dead-consumer clause removed). **All 4 condition-of-approval errata APPLIED** (`production/errata-backlog.md`): ST-1 ELZS **AC-ELZS-14** + **CP Rule 3a boss completion bonus** (Boss 1=310/Boss 2=180 — resolves OQ-CP-8, user-chosen lever); ST-2 Assembly **AC-SA-15**; ST-3 TBC **Rule 2 step 0 + AC-TBC-42**; ST-4 Consumable DB ranges [60,612]/[80,147] + **AC-CD-03 case C**. New CP ACs: AC-CP-24. Boss-bonus adds Enemy DB `completion_bonus_xp` field + TBC payload field. /consistency-check 2026-07-13 PASS (registry synced: CD-1/CD-3 ranges, SA-F1 note, NEW completion_bonus_xp constant). **Owed (non-gating, mechanical):** light `/design-review` confirmation touches on the 4 amended siblings — their Status stays APPROVED.) | design/gdd/symbot-core-progression.md | Symbot Assembly, Part Database, Turn-Based Combat, Enemy Level & Zone Scaling |
...
| 13 | World Loot System | World | MVP | Approved (2026-07-13, full-panel /design-review — 4 specialists + CD; NEEDS REVISION → 7 blockers fixed & accepted same session. B1: snapshot/restore type-contract violation vs Approved EP Rule 3 — bare Array refuses every save + wipes loot on load → wrapped in `{"collected": [...]}`. B2: `WORLD_SCRAP_CEILING = 180` + WL-PRED-2 per-zone Scrap-sum content check. B3: DDR-WL-1 (early RARE bypass accepted as intentional, capped 1/zone; economy-designer dissent logged). B4: min-1-PART-per-zone WL-PRED-2 clause (Pillar 5). B5: "already emptied" ambient cue (silent-collect UX). B6: 7 AC gaps — new Rule 9.4 `get_node_state()` + AC-WL-11 derived-visual-state assertion (highest-risk desync), per-node counting, StringName-literal/Dictionary-envelope fixtures, warning-content + sink-silence asserts. B7: Rule 5↔9.3 wording + collect() branching order. **Errata owed:** Consumable DB + Inventory downstream-reader rows; EP soft-provisional row → "Authored". **#16 obligations logged:** EC-WL-12 gesture-gating AC, refused-node nav affordance, empty-cue trigger) | design/gdd/world-loot.md | Part Database, Consumable Database, Inventory, Zone & World Map |
| 14 | Exploration Progress System | World | MVP | Approved (2026-07-13, round-2 confirmation full-panel /design-review — all 4 round-1 blockers verified fixed by all specialists; 2 new blockers found & fixed same session: MIGRATE-no-hooks → REFUSE made normative in Rule 9 + AC-EP-02(b) positive domain-state assertion; serialize structured result `{ok, failed_domain}` + Rule 3a.3 injectable warning sink for GUT testability (AC-EP-12). 2 CD-mandated fold-ins: String-cast sort normative (Rule 1/AC-EP-01), Player Fantasy OQ-EP-2 qualifier. CD verdict APPROVED, no round 3. **EZ Rule 8a erratum now due**; 8 backlog recommended items in review log) | design/gdd/exploration-progress.md | Zone & World Map, Symbot Core Progression, World Loot (#13 domain contract implemented 2026-07-13 — provisional marker discharged) |
...
| Design docs approved | 19 (Core Progression #10b **Approved** 2026-07-13 after the 4th-pass re-review + ST-1..ST-4 errata discharge + /consistency-check PASS — all 19 designed MVP GDDs now Approved) |
