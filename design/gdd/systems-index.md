...
|---|-------------|----------|----------|--------|------------|------------|
| 1 | Part Database | Foundation | MVP | **Approved** (2026-07-16, Round 11 full-panel /design-review [5 specialists + CD] re-reviewing the 7 Round-10 fixes — NEEDS REVISION: 3 new blockers [AC-25 any-key focus loophole: `{structure:10, armor:30}` Chassis Prototype passed while a pure primary-stat downgrade; AC-08(b) base −1 fixture non-discriminating vs a no-reduction implementation; `stat_bonuses` missing `Dictionary[StringName, int]` pin — String keys silently read 0]. All 3 + 7 recommended fixed same session. **User design decision: Prototype focus stat = slot primary stat** — off-primary Prototypes are not authorable in MVP. CD targeted fix-confirmation APPROVED, no re-panel. **Production debt CLOSED (2026-07-16):** story-011 (validator hardening) created, implemented, `/code-review` APPROVED WITH SUGGESTIONS (all applied), closed Complete via `/story-done` — AC-25/26/27 validators + Story-009 entry-shape checks + AC-08(b) fixture sync shipped; suite 294/294 green. Prior Round 10: NEEDS REVISION, 7 blockers, fixed same day) | design/gdd/part-database.md | — |
