...
| 10b | Symbot Core Progression (Leveling) | Gameplay | MVP | Approve-ready (2026-07-13 **4th-pass full-panel confirmation re-review** — C-2/D-2 re-confirmed RESOLVED; all 4 formulas re-verified. CD: **design is sound.** 2 in-doc fixes applied (AC-CP-21 float-cast + DF-1 ref; Rule 2/EC-CP-02 dead-consumer clause removed). **CD's condition of approval — all 4 errata DISCHARGED same session** (`production/errata-backlog.md`): ST-1 ELZS **AC-ELZS-14** equip-gate coverage + **CP Rule 3a boss completion bonus** (Boss 1=310/Boss 2=180, resolves OQ-CP-8, lever chosen by user); ST-2 Assembly **AC-SA-15** (gates AC-CP-18); ST-3 TBC **Rule 2 step 0 + AC-TBC-42** (EC-CP-05 refusal); ST-4 Consumable DB ranges [60,612]/[80,147] + **AC-CD-03 case C**. New CP AC: **AC-CP-24** (boss bonus). Boss-bonus touches Enemy DB (`completion_bonus_xp` field) + TBC (`battle_ended` payload). **Only light `/design-review` confirmation touches on the 4 mechanically-amended siblings remain** — their Status stays APPROVED, and they do not gate CP. Flip #10b to Approved on those confirmations or on the user's acceptance.) | design/gdd/symbot-core-progression.md | Symbot Assembly, Part Database, Turn-Based Combat, Enemy Level & Zone Scaling |
...
| Design docs approved | 18 (Core Progression #10b flipped to Needs Revision by /review-all-gdds 2026-07-13 for the CP-F3 range breach C-2; TBC #6 re-approved same day after cross-review erratum — see gdd-cross-review-2026-07-13.md) |
