...
| 10c | Enemy Level & Zone Scaling | World | MVP | In Review (2026-07-12, **round-3** full-panel /design-review — NEEDS REVISION → 4 blockers + 6 recommended fixed same session; all 10 round-2 items verified applied first. Round-3 fixes: AC-05 false floor discriminator → at-roof `<=` fixture + AC-05(E) all-out-of-band; AC-09 pinned `level_band(level, mid_floor, high_floor)` signature + HIGH_FLOOR retune fixture; errata pre-gate extended (canonical DS-1, AC-DS-31 level-factor fixture, AC-11 as erratum Done condition, CI roster-sweep gates CP-F4 retunes); economy relabeled ESTIMATED w/ weight provenance. CD: APPROVE on fix-confirmation, no round 5 — final confirmation pass pending. 4 errata still owed on approval: Enemy DB/Encounter Zone/Drop+economy-model/ZWM) | design/gdd/enemy-level-zone-scaling.md | Enemy Database, Encounter Zone, Drop System, Zone & World Map |
