...
Part-Break's emotional signature is the **shopping list made concrete**. In the game concept's words, every battle is a harvest decision — but that decision only has teeth when breaking a region costs something. **What it costs is turns, not damage.** Because a region-targeted hit puts only ~20% of its damage on the kill clock (spillover, Rule 4b) and the rest into a pool that never ends the fight, every turn spent harvesting is a turn *not* spent killing — and every broken region raises the enemy's enrage (Rule 7). That time-and-risk cost is systemic and build-independent: AC-PB-28 guarantees `harvest_turns > fastest_kill_turns` for **any** move bias, so even a generalist pays it. The player who routes three turns into an arm instead of finishing fast is making a bet: the Servo Arm is worth those turns, and they can survive them. When the arm shatters, the bet pays. When it doesn't — when they misjudged the enemy's counter-damage and got DOWNED a turn before the break — the lesson lands clearly: *I aimed for the arm too late.* That's the Monster Hunter DNA translated to turn tempo. The enemy is a walking shopping list; Part-Break is the mechanism that charges you — in turns and rising danger — for reading from it.

The **move bias** (Rule 3) is a second, orthogonal lever layered on top of this universal time cost: a `BREAK_HEAVY` specialist harvests those turns faster (fewer turns exposed per region) but kills slowly, a `STRUCTURE_HEAVY` "Crusher" does the reverse, and `BALANCED` — the calibration anchor — pays no efficiency penalty in either direction but forgoes the *peak* speed of a specialist in the direction it commits to. BALANCED's cost is therefore opportunity cost (never the fastest at the thing you most want this turn), while the turn-and-enrage cost above is paid by everyone. Neither the bias choice nor the anchor removes the harvest tax; they only change how steep the specialist's edge is.
...
**Rule 7 — Enrage escalator (designed-in; content-gated).** Each broken region raises the enemy's **outgoing-damage multiplier** by `ENRAGE_PER_BREAK = 0.12` (additive per broken region, applied by TBC to the enemy's final damage output, persists for the battle, resets at battle end; max +36% at 3 breaks). Greedy multi-break therefore grows progressively more dangerous — the `all_boss_parts_broken` capstone is fought against a maximally enraged enemy. Setting `ENRAGE_PER_BREAK = 0` disables it globally; the always-on lever is region-fraction cost (Rule 1 / `EDB-1`). MVP default value set in Formulas / Tuning Knobs.
...
The enum enforces the trade: no bias is strong against both pools. The ratio `structure_mult : break_mult` is 2.27× for Crusher and 2.0× for Breaker — keep both ≥ 2.0× or the specialist collapses toward Balanced. **The ratio alone is not a sufficient guard, however:** it is preserved by scaling both multipliers down together (e.g. Breaker at `(0.60, 1.20)` still meets 2.0× but breaks 14% slower and reads closer to Balanced at typical `move_damage`). So the ratio constraint is paired with an **absolute-floor companion rule**: the specialist's *strong* multiplier must stay ≥ 1.35 (Breaker `break_mult`) / ≥ 1.20 (Crusher `structure_mult`) so the specialist edge is felt at mid `move_damage` (~30–80), not only at the one-shot ceiling. Retuning must satisfy **both** the ratio and the absolute floor.
...
| PB-F5 | 1.12, 1.24, 1.36 (`ENRAGE_PER_BREAK = 0.12`) | defensive | none — all three enrage multipliers empirically clean at 0.12 (re-scanned 2026-07-11 after retune from 0.15). The former load-bearing 1.15 case existed only at the old 0.15 value and is gone. |
...
```
Applied by TBC to the **enemy's** outgoing hit (Rule 7), after the enemy's own DF-1/pipeline resolution, before it reduces a player Symbot's `current_structure`. `ENRAGE_PER_BREAK = 0.12`.
...
| Enrage per break | `ENRAGE_PER_BREAK` | float | 0.12 | Tuning knob; additive per break |
| Damage floor | `DAMAGE_FLOOR` | int | 1 | Shared constant |
| Output | `enraged_damage` | int | 1–428 | Multiplier range [1.00, 1.36]; `floor(315 × 1.36 + ε) = 428` at theoretical ceiling |

**Worked example (rounding-discriminating):** `enemy_hit_resolved = 73`, `broken_region_count = 2` → mult 1.24: `max(1, floor(73 × 1.24 + 0.0001)) = max(1, floor(90.5201)) = 90` — round/ceil give 91.
**Worked example (rounding-discriminating, max stack):** `enemy_hit_resolved = 41`, `broken_region_count = 3` → mult 1.36: `max(1, floor(41 × 1.36 + 0.0001)) = max(1, floor(55.7601)) = 55` — round/ceil give 56. *(Post-retune to 0.12, the PB-F5 multipliers {1.12, 1.24, 1.36} are all epsilon-**defensive** — scan-verified 2026-07-11, see Section D. The former load-bearing 1.15 case existed only at the old 0.15 value.)*

**Calibration (honest bound):** enrage is a **greed tax that scales with how much you dismantle**, capped at +36% (3 breaks). Against a mid enemy whose resolved hit is ~40, full enrage raises it to `floor(40 × 1.36 + ε) = 54` — noticeable but absorbable by a ~200-Structure Symbot (≈4 hits-to-down). **Minimum-Structure glass cannons (SA-F1 floor = 60) are a different story:** a realistic ~55 enemy hit already leaves only a 5-point margin at zero enrage, and *any* enrage stack tips it over — `floor(55 × 1.12 + ε) = 61 > 60` one-shots at a single break. This is **intended risk, not a bug**: a 60-Structure build is not expected to safely full-dismantle a threatening enemy — that is the fantasy's "can I survive?" tension made real. The exact survivability of a low-Structure build depends on its own defense mitigation inside DF-1 (a cross-system variable Part-Break does not own), so the precise glass-cannon threshold is playtest-gated — see **OQ-PB-3**. Retuning from the prior 0.15 to 0.12 lowers the cap from +45% to +36%, widening the tanky-build absorption band without pretending the glass build is safe. The `all_boss_parts_broken` capstone is thus fought against a maximally enraged enemy.
...
| `ENRAGE_PER_BREAK` | 0.12 | 0.08 – 0.20 | Additive enemy outgoing-damage bonus per broken region (PB-F5); max total = 3× this at full dismantle (+36% at 0.12). At 0.20 (max +60%) a fully-enraged enemy two-shots — and often one-shots — glass-cannon Symbots; at 0.08 (max +24%) enrage is cosmetic and full-dismantle becomes near-free loot. **Calibration caveat:** even at the current 0.12, a minimum-Structure build (SA-F1 floor 60) is one-shot by a realistic ~55 enemy hit at a *single* enrage stack (55×1.12 = 61 > 60) — the thin margin is the base-hit-vs-Structure-floor, not the multiplier. Lowering `ENRAGE_PER_BREAK` widens the *tanky*-build absorption band but does not make the 60-floor build safe; that is intended (see PB-F5 Calibration + OQ-PB-3). **Re-run the epsilon scan if changed** (new multipliers). |
...
1. **Any change to a bias multiplier, `BREAK_SPILLOVER`, or `ENRAGE_PER_BREAK` invalidates the epsilon scan** — the load-bearing/defensive split (PB-F1@0.70, PB-F2@1.40 load-bearing; PB-F5 all-defensive at 0.12; rest defensive) is coefficient-specific. Re-run the python3 scan and update Section D.
2. **`ENRAGE_PER_BREAK` is coupled to player Structure ranges** — it is balanced against SA-F1's Structure output (60–594) and the DF-1 enemy-damage band. At the current 0.12 the minimum-Structure build (60) is already one-shot by a realistic ~55 enemy hit at a single enrage stack (intended; see OQ-PB-3). Retuning enemy power (EDB-2) or Structure ranges shifts what enrage "feels like"; always re-check the glass-cannon case (low-Structure builds vs. max enrage) before shipping a change.
...
**UI-3 — The harvest tilt must be legible (Pillar-2 wiring), gated behind SCAN.** The UI must make it visible that targeting a region is *worth it* by surfacing which part a region's break helps drop (ties to Enemy DB ED6 / the Move DB SCAN reveal). **Reveal model — hidden until SCAN (decided 2026-07-11):** before the enemy has been scanned, a region target shows its **name and break-progress pip only** — no drop information. Spending a turn on a SCAN move reveals, per region, which part its break helps drop. This makes SCAN a real strategic action (a turn traded for harvest intelligence) rather than free information, and preserves the discovery layer Pillar 5 wants. **Onboarding caveat:** because a pre-SCAN player sees no drop tilt, the Tutorial System (Vertical Slice) must teach SCAN-before-harvest explicitly, or a first-time player will target regions blindly; flag this as a tutorial dependency, not a Part-Break runtime concern. This is the downstream half of the "visible tilt" promise the Drop System's fantasy depends on.
...
**AC-PB-04 (PB-F5 enrage damage).** **[Logic, BLOCKING]** *(a) Rounding discriminator:* **GIVEN** `enemy_hit_resolved = 73`, `broken_region_count = 2` (×1.24), **WHEN** the enemy attack resolves, **THEN** `enraged_damage == 90` (exact 90.52; round/ceil give 91). *(b) Rounding discriminator, max stack:* **GIVEN** `enemy_hit_resolved = 41`, `broken_region_count = 3` (×1.36), **THEN** `enraged_damage == 55` (exact 55.76; round/ceil give 56). *(Post-retune to `ENRAGE_PER_BREAK = 0.12`, no epsilon-regression fixture exists for PB-F5 — all three multipliers {1.12, 1.24, 1.36} are scan-verified defensive; see Section D.)* *(c) Identity:* **GIVEN** `broken_region_count = 0`, **THEN** `enraged_damage == enemy_hit_resolved` (multiplier exactly 1.0).

**AC-PB-05 (PB-F1/F2/F3 DAMAGE_FLOOR clamp).** **[Logic, BLOCKING]** *(a)* **GIVEN** `move_damage = 1`, `STRUCTURE_HEAVY` (`break_mult = 0.55`), **WHEN** a region hit resolves, **THEN** `break_damage == 1` (`floor(0.55) = 0`, clamped up). *(b)* **GIVEN** `move_damage = 1`, `BREAK_HEAVY` (`structure_mult = 0.70`), **THEN** `structure_damage == 1`. *(c)* **GIVEN** `move_damage = 1`, `STRUCTURE_HEAVY` (`break_mult = 0.55`), region-target, **THEN** **both** `break_damage == 1` (`floor(0.55) = 0`, clamped up) **and** `spillover_damage == 1` (`floor(0.11) = 0`, clamped up) on the same hit — asserting both floors together, so an impl that clamps one but not the other fails. *(d) DAMAGE_FLOOR-driven kill path:* **GIVEN** enemy `current_structure = 1`, a region at `current_break_hp = 5`, and the same `move_damage = 1` `STRUCTURE_HEAVY` region hit, **THEN** the clamped `spillover_damage == 1` drops Structure `1 → 0` (VICTORY), the region is **not** broken (`current_break_hp == 4`), and no `<region>_broken` fires — the EC-PB-03 kill-via-floor path is exercised, not just the clamp value.
...
**AC-PB-09 (all_boss_parts_broken emission).** **[Logic, BLOCKING]** *(a) Multi-region:* **GIVEN** an enemy with `N = 3` breakable regions, **WHEN** the last INTACT region breaks, **THEN** both `<region>_broken` and `all_boss_parts_broken` are emitted, and `broken_region_count == N` at emission. **WHEN** fewer than `N` are broken, `all_boss_parts_broken` is absent. *(b) Single-region boundary (N = 1):* **GIVEN** an enemy with exactly **one** breakable region, **WHEN** that region breaks, **THEN** `<region>_broken` **and** `all_boss_parts_broken` fire together in the same resolution and `broken_region_count == 1`. *(Catches an off-by-one "all broken" guard such as `count > 1` that passes for N ≥ 2 but fails at N = 1.)*
...
**AC-PB-21 (kill before break).** **[Logic, BLOCKING]** **GIVEN** a region INTACT and the **precondition** enemy `current_structure == 10`, **WHEN** a `STRUCTURE_HEAVY` Structure-target hit (`move_damage = 8` → `structure_damage = floor(8 × 1.25 + ε) = 10`) resolves, **THEN** enemy `current_structure == 0` (assert the postcondition, not merely "VICTORY fired"), the battle ends VICTORY, the region stays INTACT (`is_broken == false`), and its `<region>_broken` key is absent from `fired_break_events`. *(The precondition + exact-Structure postcondition catch an impl that fires VICTORY on any hit regardless of the Structure value. The fixture is behavioral, not a rounding test — `floor(10) = round(10) = ceil(10)`.)*
...
**AC-PB-23 (spillover kills without breaking).** **[Logic, BLOCKING]** **GIVEN** a region `break_hp = 100`, enemy `current_structure == 3`, **WHEN** a `BREAK_HEAVY` region hit `move_damage = 50` resolves, **THEN** assert **all** of: `break_damage == 70` (< 100), `spillover_damage == floor(50 × 1.40 × 0.20 + ε) == 14` (the kill vector — assert it explicitly, not just the outcome), enemy `current_structure == 0` after the spillover, the battle ends VICTORY, the region is NOT broken (`current_break_hp == 30`), and no `<region>_broken` is emitted. *(Asserting `spillover_damage == 14` catches an impl that computes spillover from HP depleted rather than raw `move_damage`, or that double-applies it.)*

**AC-PB-24 (break progress evaporates on non-victory).** **[Integration, BLOCKING]** **GIVEN** broken regions and non-zero enrage mid-battle, **WHEN** the player flees (scenario A) **or** is defeated (scenario B), **THEN** all region pools, `is_broken` flags, `broken_region_count`, and `fired_break_events` are discarded, and a fresh encounter with the same enemy starts every region INTACT with `broken_region_count = 0`. **AND** on that fresh encounter `fired_break_events` is a **new empty set** (not null): the first region-break of the next battle emits its key successfully without a null-reference error. *(Catches an impl that clears the old set but forgets to re-initialize it, crashing on the next battle's first break.)*
...
**AC-PB-28 (Pillar-2 harvest cost — BINDING).** **[Integration, BLOCKING]** *Fixture prerequisite: requires the TBC integration harness (action-consumption loop, victory check, and spillover application wired) — this is not a Part-Break-isolated unit test. `ENRAGE_PER_BREAK` is irrelevant to the result (enrage modifies enemy outgoing damage, not the player's route), so the assertion holds at any enrage value.* **GIVEN** the reference fixture — enemy Structure 60 with 3 regions each `break_hp = 30`, one move `move_damage = 30`, unlimited uses — **WHEN** Route A (STRUCTURE_HEAVY on Structure: 37 dmg/turn) and Route B (BREAK_HEAVY breaking all 3 regions, +8 spillover each, then finishing at 21/turn) are each run with identical Symbot config, **THEN** Route A reaches VICTORY in **2** turns and Route B in **5** turns, and the assertion `harvest_turns (5) > fastest_kill_turns (2)` holds. Discharges the TBC BINDING Pillar-2 obligation: part-targeting is never free relative to fastest kill.
...
**OQ-PB-3 — `ENRAGE_PER_BREAK = 0.12` playtest validation (glass-cannon threshold).** The value was lowered from 0.15 → 0.12 in review (2026-07-11) to widen the tanky-build absorption band (cap +45% → +36%). It remains analytically calibrated against Structure ranges; the **known-and-intended** consequence is that a minimum-Structure build (SA-F1 floor 60) is one-shot by a realistic ~55 enemy hit at even a single enrage stack (61 > 60). Whether that risk *feels* fair versus punishing across archetypes — and whether the true threshold shifts once a low-Structure build's own DF-1 defense mitigation is in play (a cross-system variable) — is the open playtest question. If glass-cannon full-dismantle feels impossible in a bad way, the lever is per-enemy enrage gating (OQ-PB-4) or the SA-F1 Structure floor, **not** further lowering `ENRAGE_PER_BREAK` (which barely helps at the 60-floor margin). *Owner: playtest / balance-check.*
