...
| `completion_bonus_xp` | int | **Core Progression erratum ST-1 (2026-07-13).** Flat XP bonus **added to `xp_value`** by Core Progression at battle end (CP Rule 3a) before the deployed/benched split, **but only on the boss's FIRST-EVER defeat** (`is_first_boss_defeat == true`; enforced by CP Rule 3a's first-defeat guard, verified by AC-CP-25 — a `LIGHTER_REGATE` boss refight awards `xp_value` alone, never the bonus, so it can't be farmed). **`0` for all WILD enemies** (default); non-zero **only on BOSS entries**, where it is sized to land the deployed core at the equip gate for the rarity the boss drops (the OQ-CP-8 fix). Unlike `xp_value` it is **not** CP-F4-derived — it is a per-boss authored value calibrated by the ELZS pass against the equip-gate coverage AC (AC-ELZS-14). **MVP values: Boss 1 = 310, Boss 2 = 180; all WILD = 0.** Content validation (BLOCKING): must be `≥ 0`, and must be `0` unless `enemy_class == BOSS`. *(See Core Progression Rule 3a + AC-CP-25 + ELZS AC-ELZS-14.)* |
