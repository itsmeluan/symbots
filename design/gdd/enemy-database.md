...
| `completion_bonus_xp` | int | **Core Progression erratum ST-1 (2026-07-13).** Flat, one-time XP bonus awarded on defeating this enemy, **added to `xp_value`** by Core Progression at battle end (CP Rule 3a) before the deployed/benched split. **`0` for all WILD enemies** (default); non-zero **only on BOSS entries**, where it is sized to land the deployed core at the equip gate for the rarity the boss drops (the OQ-CP-8 fix). Unlike `xp_value` it is **not** CP-F4-derived — it is a per-boss authored value calibrated by the ELZS pass against the equip-gate coverage AC (AC-ELZS-14). **MVP values: Boss 1 = 310, Boss 2 = 180; all WILD = 0.** Content validation (BLOCKING): must be `≥ 0`, and must be `0` unless `enemy_class == BOSS`. *(See Core Progression Rule 3a + ELZS AC-ELZS-14.)* |
