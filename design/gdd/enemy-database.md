...
7. **Minimum-meaningful break multiplier (owner: Drop System GDD / Part DB; resolve: before Boss-grade loot is authored):** AC-ED-03's `loot_connected` check is syntactic-only — it verifies that a break event appears in at least one pool part's drop_conditions, but does not inspect the multiplier value. A multiplier of ×1 satisfies the check but provides no harvest incentive (drop rate unchanged from base rate). The minimum multiplier that creates meaningful incentive (i.e., a player who executes the break perceives a materially higher drop chance) has not been defined. The Drop System GDD or Part DB should specify a floor (e.g., "break multipliers < ×10 are flagged as likely unintentional"). Without this, authors can satisfy loot_connected and Pillar 2 syntactically while providing no actual hunting incentive.
