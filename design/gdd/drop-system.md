...
> **Last Updated**: 2026-07-11 (NEEDS REVISION addressed: DS-2 partial-credit pity model, States-table pity ordering fold, AC-DS-25 floor-compliant multiplier, AC-DS-13/14/15/26 credit-model + discriminator fixes, new AC-DS-29 partial-credit, AC-DS-10/18/21/24 multi-part coverage, Boss-grade 0% absorption + game-thirds economy, scrap-yield range invariant, OQ-DS-6 accepted victory-only)
...
On a NON-qualifying attempt (c == 0, no conditions fired): pity_credit[p] unchanged (no credit — anti-exploit; an unrelated fight banks nothing).
```
...
|------|-------|-----------|-----------------------|
| `N_PROTO_PITY` | 25 | 15–25 | Prototype pity base (DS-2), in optimal-attempts-to-guarantee; the runtime credit threshold is `N_PROTO_PITY × C` (C = the part's condition count). Lower → pity fires more often (becomes an expected path below ~12); higher → rarer safety net. At 25, ~1.0% of hunters hit it via optimal play at the typical optimal rate (0.169) and ~1.72% at the authored floor (0.15) — see Pity Calibration Authoring Rules. Partial play converges at `N_PROTO_PITY × C / c` attempts. |
...
Absorption is the sensitive lever, so state a **band: ~1,600–2,150 Scrap** (Common/Rare absorption ±10pp; Boss-grade held at 0). Common yield alone is ~57% of the faucet — consistent with the Tuning Knobs table calling it the highest-leverage faucet knob. The band's **floor is the riskier tail**: under-absorption (banking Commons for future Symbots) is more likely than over-absorption, especially before hour 5.

**Faucet over the arc (game-thirds sketch — timing, not just total).** Because early Common absorption runs low (A3 temporal caveat), Scrap income is back-loaded rather than linear. Rough distribution of the ~1,840 total:

| Arc third | ~Victories | Symbots owned | Common absorption | ~Scrap earned | What the player can afford |
|-----------|-----------|---------------|-------------------|---------------|-----------------------------|
| Hours 1–3 | ~60 | 1 | ~15% | **~250** | First cheap upgrades (10/20/40) on the lead Symbot's priority parts — the "upgrade hook" lands |
| Hours 4–7 | ~80 | 2 | ~60% | **~680** | Mid-tier upgrades on lead + starting the second Symbot; scarcity becomes felt |
| Hours 8–10 | ~60 | 3 | ~90% | **~910** | The expensive +4→+5 tiers (80/160) on priority parts; third Symbot partially kitted |

The sketch confirms the cheap early tiers (10/20/40 = 70 total for a Common max) are affordable inside the first third's ~250 Scrap, so the upgrade habit forms early even at low early absorption — the economy is not dead at the start. The expensive tiers correctly land in the final third when income peaks. **Watch criterion (OQ-DS-5):** if hours 1–3 real income falls below ~120 Scrap (absorption near 0%, player hoards everything), the player cannot afford even the first +0→+1 (10) comfortably and the hook is delayed — playtest for "does the player make their first upgrade before hour 3?"
...
**Result — mild scarcity holds.** Faucet ~1,840 (band 1,600–2,150) ≈ **just under two full core loadouts** (~1.8 × ~1,000); the lead Symbot is fully kitted, the second is nearly there, and the third's loadout (~1,000 more) is only partially funded. So the player **fully kits their lead Symbot and most of a second, then chooses which parts on the third to prioritize** — upgrade-target choices stay meaningful without a grind wall. The "just under two" (rather than a clean two) is deliberate: it means even a 2-Symbot player must make a few priority calls near the end rather than maxing everything.
...
| OQ-DS-5 | **Scrap economy validation (assumptions A1–A3).** The rederived faucet (~1,600–2,150 Scrap band) vs full 3-Symbot priority sink (~3,000) targets mild scarcity, but rests on design-time assumptions: battle volume (~200 victories), per-victory yield, and the drop-absorption rates (Common 75% / Rare 50% / Prototype 25% / Boss-grade 0%). **Watch criteria at playtesting:** (a) actual battle count and absorption behavior vs A1/A3 — if absorption runs high, faucet approaches the sink and scarcity softens; (b) <3 Symbots → Scrap saturates early (check upgrade choices stay meaningful past hour 5); (c) >3 Symbots → scarcity extends; (d) **early-game timing** — does the player make their first upgrade before hour 3? (the game-thirds sketch assumes ~250 Scrap in hours 1–3 at ~15% Common absorption; if real early absorption is near 0%, the upgrade hook is delayed); (e) **Prototype saturation** — with only ~1–2 distinct Prototypes acquired over the arc, at what hour does the player exhaust unique-Prototype hunt goals? If before hour 7, MVP may need an additional Prototype part or an Alpha teaser to keep the Prototype hunt alive. | Playtesting + Workshop/Inventory GDD (must fix total Symbot count & upgrade-cost curve) | Economy model precision; mild scarcity may break at extreme Symbot counts, off absorption assumptions, or early-timing skew |
