...
**Rule 4 — Effective rate (evaluates Part DB Formula 3).** Per rolled part: `effective_drop_rate = clamp(base_drop_rate[rarity] × Π matching-condition multipliers × level_rarity_mult × beacon_factor, 0, 1)`. Base rates from Part DB config: Common 0.70, Rare 0.25, Boss-grade 0.001, Prototype 0.05. **This is the canonical amended DS-1** — it includes `level_rarity_mult` (DS-F-LEVEL, ELZS erratum; = 1.0 when no level scaling) and `beacon_factor` (CD-4 Beacon; = 1.0 when no Beacon active). **Do not code the base×conditions form alone** — dropping the level factor fails AC-ELZS-10/AC-ELZS-11 (returns 0.375 for all bands instead of the 0.1875/0.375/0.5625 EARLY/MID/HIGH spread). Full formula, variable table, and worked examples in Formulas (canonical DS-1) + the Beacon/level injection notes.
