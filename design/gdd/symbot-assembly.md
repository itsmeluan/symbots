...
**AC-SA-15** — CP-F3 level-growth is inserted AFTER SA-F1 and BEFORE synergy (Rule 6 step 4b insertion order). *(Core Progression erratum ST-2, 2026-07-13 — the enforcement gate for the CP-F3 step this GDD added in Rule 6 step 4b.)*
Setup: chassis archetype multiplier `M = 1.2` on `target_stat`; the equipped CORE has `level_growth = { target_stat: 10 }` and is at **level 5** (CP-F3 contribution = `10 × (5−1) = 40`); the equipped parts produce an SA-F1 output of **120** for `target_stat` (100 raw `stat_bonuses` × 1.2 archetype). **Pass when**: the `final_stat["target_stat"]` value exposed to downstream synergy (SYN-F4) is exactly **160** (= 120 + 40 — CP-F3 added flat, after the chassis multiply), **NOT 168** (= (100 + 40) × 1.2, which results from inserting CP-F3 *before* SA-F1 so the chassis modifier wrongly amplifies level growth), and **NOT** any value reflecting CP-F3 applied after synergy. This proves step 4b bypasses the chassis modifier and precedes synergy. **Test type**: Integration (exercises the full equip → SA-F1 → CP-F3 → pre-SYN-F4 handoff).
> **DoD gate (binding — qa-lead R3-C / R4-B1, ST-2).** This AC **is the same test as Core Progression AC-CP-18** (which is DEFERRED there, awaiting this insertion). Passing AC-SA-15 is a **required Definition-of-Done item** on the story that wires the Rule 6 step 4b CP-F3 contribution: the story MUST NOT be marked complete with the CP-F3 step inserted but this ordering untested, because a wrong insertion point (before SA-F1, or after synergy) produces a different `final_stat` that no other non-deferred AC catches. Cross-ref: `symbot-core-progression.md` AC-CP-18.
