...
- **APPLIED 2026-07-13** (design-review session — **lever chosen by user: per-boss completion bonus**).
  ...
  - **REFIGHT-GUARD FOLLOW-UP (added 2026-07-13, post-Approval /review-all-gdds):** the boss bonus lacked a first-defeat guard → repeated on `LIGHTER_REGATE` refights (blocking anti-treadmill defect). **Fixed same session:** CP **Rule 3a first-defeat guard** gated on new `battle_ended.is_first_boss_defeat: bool` (TBC Rule 12 eight-field payload); **AC-CP-25** (refight awards no bonus) + **EC-CP-13**; propagated to Enemy DB field note + ELZS AC-ELZS-14 + registry. The `is_first_boss_defeat` provenance (boss-approach computes it from ZWM `defeated_once` pre-battle) is a forward-reference to **Overworld Navigation #16 (Not Started)** — when authored, ON#16 must supply this flag to TBC at boss-battle start. Logged as an ON#16 obligation.
