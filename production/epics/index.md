...
| [Drop System](drop-system/EPIC.md) | Core | Drop System | design/gdd/drop-system.md | 9 stories | ✅ 8/9 (DS-009 Blocked on Save/Load) |
...
- **Core** — ✅ **fully Complete as of 2026-07-17** (5 epics, all closed through the lean per-story `/code-review` + `/story-done` gate): Symbot Assembly (7/7), Synergy System (5/5), Turn-Based Combat (14/14), **Encounter Zone (8/8, closed 2026-07-17)**, and **Drop System (8/9, closed 2026-07-17 — DS-009 Blocked on the Not-Started Save/Load system)**. **Full suite 869/869 GUT green, 4606 asserts.** Encounter Zone shipped `EncounterResolver` + `ZoneContentLinter` in `src/core/encounter_zone/` (value types `ZoneDef`/`TerrainPatch`/`SpawnEntry`/`BossEncounter`; EZ-1 rate clamp, EZ-2 weighted walk, sub-pool `filter_valid`, WILD/BOSS TBC handoff, WIN_COUNT first-access + `requires_defeated` sequencing, LIGHTER_REGATE delta re-gate + ALWAYS_OPEN, gate-param validation + reserved-gate fail-safe LOCKED, and 11 ADVISORY offline content linters proven against fixtures) — the real MVP zone `.tres` remains a deferred content pass (OQ-EZ-1). Drop System shipped its 8 Logic stories (all 12 TR-drop requirements + 30 numbered BLOCKING ACs); DS-009 Integration (pity-counter persistence AC-DS-28) stays **Blocked** on Save/Load (ADR-0001 must define the pity-map serialization interface) — a release-blocker before ship. Deferred integration ACs (EZ 9, Drop 4) recorded as write-the-stub-now notes; the DS-F-LEVEL cross-system gate (AC-ELZS-11) is discharged by the Encounter Zone erratum. All traced to Accepted ADRs (ADR-0005 stat pipeline, ADR-0006 RNG/determinism, ADR-0007 TBC FSM, ADR-0001/0002/0003). 0 untraced Core requirements. **Next: the Technical Setup → Pre-Production gate** (`/test-setup`, `/ux-design`), plus landing Save/Load (ADR-0001) to unblock DS-009.
...
Drop System (8/9, DS-009 Blocked on Save/Load). **Full suite 869/869 GUT green, 4606 asserts.**
Every Core system is built and tested against DI seams + fixtures; the two deferred content passes
(the MVP zone `.tres`, OQ-EZ-1; the synergy tier `.tres`) are authored later against the same linters.
